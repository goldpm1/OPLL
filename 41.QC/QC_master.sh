...
DATA_PATH="/data/project/OPLL/04.WES/01.QC"
...
for idx in ${!sample_name_LIST[@]}; do
    ...
    for TISSUE in Blood LF Lamina OLF; do
        FASTQ_PATH_1=$DATA_PATH"/00.raw/"$TISSUE"/"$Sample_ID"_"$TISSUE"_1.fastq.gz"
        FASTQ_PATH_2=$DATA_PATH"/00.raw/"$TISSUE"/"$Sample_ID"_"$TISSUE"_2.fastq.gz"
        ...
        if [ -f "$FASTQ_PATH_1" ]; then     # File이 있어야만 진행
            ...
            FASTP_PATH_1=$DATA_PATH"/01.fastp/"$TISSUE"/"$Sample_ID"_"$TISSUE".R1.fastq"
            FASTP_PATH_2=$DATA_PATH"/01.fastp/"$TISSUE"/"$Sample_ID"_"$TISSUE".R2.fastq"
